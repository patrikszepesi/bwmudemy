  ...
  renderBookings(bookings) {
    ...
    return bookings.map((booking, index) =>
      ...
      <React.Fragment key={index}>
        ...
        <p><span>Diák Neve:</span> {booking.guests}</p>
        <p><span>Elérhetőség:</span> {booking.usrContact} </p>
        <p><span>Hol Tanul:</span> {booking.about} </p>
        <p><span>Miből Kell Korrepetálni:</span>  {booking.what}</p>
